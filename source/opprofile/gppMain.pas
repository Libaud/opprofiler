...
{$ENDIF}
  Registry, Messages, Classes, Forms, SysUtils, Graphics, Controls,
  Dialogs, StdCtrls, Menus, ComCtrls, gpParser, ExtCtrls, gpCheckLst, gpMRU,
  ActnList, ImgList, Buttons, ToolWin, gppResults, Grids, FileUtil,
...
begin
  if not FileExistsUTF8(fileName) { *Converti depuis FileExists* } then
  begin
...
begin 
  if not FileExistsUTF8(fileName) { *Converti depuis FileExists* } then StatusPanel0('File '+fileName+' does not exist!',false,true)
  else begin
...
begin
  if assigned(openProject) then begin
    ...
    if not FileExistsUTF8(vFName) { *Converti depuis FileExists* } then
    begin
...
{$ENDIF}
  ...
  if not FileExistsUTF8(Application.HelpFile) { *Converti depuis FileExists* } then Application.HelpFile := '';
  LoadLayouts;
...
begin
  ...
  if FileExistsUTF8(fnm) { *Converti depuis FileExists* } then
  ...
  begin
    if ParamCount <> 0 then begin
      ...
      if (ParamCount > 1) or (defDelphi = '') then begin
        ...
        if FileExistsUTF8(dpkName) { *Converti depuis FileExists* }
...
begin
  ...
  if FileExistsUTF8(vDofFN) { *Converti depuis FileExists* } then
  ...
  if FileExistsUTF8(vDProjFN) { *Converti depuis FileExists* } then
  begin
  ...
  if FileExistsUTF8(vBdsProjFN) { *Converti depuis FileExists* } then
  begin
  ...
  Result := '';
  vOldCurDir := GetCurrentDirUTF8; { *Converti depuis GetCurrentDir* }
  if not SetCurrentDirUTF8(ExtractFileDir(aProject)) { *Converti depuis SetCurrentDir* } then
    Assert(False);
  try
    for i := 1 to NumElements(vPath, ';', -1) do
    begin
      vFullPath := ExpandUNCFileNameUTF8(NthEl(vPath, i, ';', -1)); { *Converti depuis ExpandUNCFileName* }
      if DirectoryExistsUTF8(vFullPath) { *Converti depuis DirectoryExists* } then
        Result := Result + IfThen(Result <> '', ';') + vFullPath;
    end;
  finally
    SetCurrentDirUTF8(vOldCurDir); { *Converti depuis SetCurrentDir* }
  end;
...
begin
  ...
  begin
    ...
    try
      if not SetCurrentDirUTF8(ExtractFileDir(aProject)) { *Converti depuis SetCurrentDir* } then
        Assert(False);
      Result := ExpandUNCFileNameUTF8(Result); { *Converti depuis ExpandUNCFileName* }
    finally
      SetCurrentDirUTF8(vOldCurDir) { *Converti depuis SetCurrentDir* }
    end;
  end;
...
begin
  with delphiProcessInfo do begin
    if hProcess <> 0 then begin
      ...
      FileClose(hThread); { *Converti depuis CloseHandle* }
      hProcess := 0;
      hThread  := 0;
    end;
    delphiThreadID   := 0;
...
begin
  if undelProject = '' then begin // delete
    ...
    RenameFileUTF8(openProfile.Name,undelProject); { *Converti depuis RenameFile* }
    NoProfile;
    SwitchDelMode(false);
  end
  else begin
    newProj := ChangeFileExt(undelProject,'.prf');
    RenameFileUTF8(undelProject,newProj); { *Converti depuis RenameFile* }
    LoadProfile(newProj);
  end;
end;
...
begin
  if delete then begin
    if undelProject <> '' then DeleteFileUTF8(undelProject); { *Converti depuis DeleteFile* }
    undelProject := '';
...
begin
  with SaveDialog1 do begin
    ...
    if Execute then begin
      if ExtractFileExt(FileName) = '' then FileName := FileName + '.prf';
      RenameFileUTF8(openProfile.Name,FileName); { *Converti depuis RenameFile* }
