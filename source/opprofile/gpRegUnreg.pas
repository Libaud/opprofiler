...
{$IFDEF LogRegister}GpLogEventEx('RegisterGpProfile out',FullLogName); {$ENDIF}  
  ...
  var
    ...
    S  : TSearchRec;
  begin
    err := FindFirstUTF8(folder+'\*.*',0,S); { *Converti depuis FindFirst* }
    if err = 0 then begin
      repeat
        SetFileAttributes(PChar(folder+'\'+S.Name),0);
        DeleteFileUTF8(folder+'\'+S.Name); { *Converti depuis DeleteFile* }
        err := FindNextUTF8(S); { *Converti depuis FindNext* }
      until err <> 0;
