...
{========================= TUnit =========================}
  ...
  begin
    ...
    if FileExistsUTF8(aUnitName) { *Converti depuis FileExists* } then
    begin
      aUnitFullName := LowerCase(ExpandUNCFileNameUTF8(aUnitName) { *Converti depuis ExpandUNCFileName* });
      Result := True;
      Exit;
    end;
    ...
    if FileExistsUTF8(vDefDir + aUnitName) { *Converti depuis FileExists* } then
    begin
    ...
    begin
      ...
      if FileExistsUTF8(s) { *Converti depuis FileExists* } then
      begin
    ...
    begin
      if aLocation = '' then // path to unit in dpr-file not specified
        ...
        if IsAbsolutePath(vLocation) then
          ...
          Result := ExpandFileNameUTF8(MakeBackslash(ExtractFilePath(aProject.prUnit.unFullName)) + vLocation); { *Converti depuis ExpandFileName* }
      end;
    end; { ExpandLocation }
...
{$IFDEF LogParser}GpLogEvent(Format('Parsing: %s', [unFullName]), FullLogName);{$ENDIF}
      unFileDate        := FileAgeUTF8(unFullName); { *Converti depuis FileAge* }
      state             := stScan;
      stateComment      := stNone;
      implement         := false;
      block             := 0;
  ...
  begin { TUnit.Instrument }
    ...
    DeleteFileUTF8(justName + '.bk2'); { *Converti depuis DeleteFile* }
    RenameFileUTF8(justName + '.bk1',justName + '.bk2'); { *Converti depuis RenameFile* }
...
{========================= TProject =========================}
  ...
  begin
    ...
    try
      ...
      if not SetCurrentDirUTF8(ExtractFilePath(prUnit.unFullName)) { *Converti depuis SetCurrentDir* } then
        Assert(False);
      try
        un := prUnit;
        repeat
  ...
  begin
    ...
    try
      ...
      try
        vOldCurDir := GetCurrentDirUTF8; { *Converti depuis GetCurrentDir* }
        if not SetCurrentDirUTF8(ExtractFileDir(prUnit.unFullName)) { *Converti depuis SetCurrentDir* } then
          Assert(False);
        try
          with prUnits do begin
            anyInst := false;
      ...
      if not anyInst then
      begin
        DeleteFileUTF8(aIncFileName); { *Converti depuis DeleteFile* }
        DeleteFileUTF8(ChangeFileExt(aIncFileName,'.gpd')); { *Converti depuis DeleteFile* }
      end;
    ...
    try
      ...
      begin
        ...
        begin
          un := TUnit(Examine(cursor));
          if (not un.unExcluded) and (un.unProcs.Count > 0) and
             (aIgnoreFileDate or (un.unFileDate <> FileAgeUTF8(un.unFullName) { *Converti depuis FileAge* })) then
  ...
  begin
    ...
    with prUnits do begin
      ...
      while not IsAfterLast(cursor) do begin
        ...
        if (not un.unExcluded) and (un.unProcs.Count > 0) and
           (un.unInProjectDir or (not projectDirOnly)) then
          if un.unFileDate <> FileAgeUTF8(un.unFullName) { *Converti depuis FileAge* } then
            Exit;
        cursor := Next(cursor);
      end;
    end;
