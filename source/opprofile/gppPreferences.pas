...
uses
{$IFnDEF FPC}
  ShellAPI, ShlObj, Windows,
{$ELSE}
  LCLIntf, LCLType, LMessages,
{$ENDIF}
  Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, ComCtrls, ExtCtrls, Menus, ImgList, ActnList, FileUtil;
...
var
  ...
  begin
    res := FindFirstUTF8(MakeBackslash(vSelDir)+mask,0,S); { *Converti depuis FindFirst* }
    if res = 0 then begin
      repeat
        AddUnit(UpperCase(FirstEl(S.Name,'.',-1)));
        res := FindNextUTF8(S); { *Converti depuis FindNext* }
      until res <> 0;
      FindCloseUTF8(S); { *Converti depuis FindClose* }
    end;
  end; { Iterate }
